...
offset_start = ddp_rank * 10
offset_end = offset_start + 10
assert offset_end <= 100, f"offset_end({offset_end}) must be less than 100"
tprint(f"加载方式处理数据集，只下载和处理4-5评分范围的高质量内容. 第{ddp_rank}个进程，从{offset_start}%到{offset_end}%")
dataset = load_dataset("opencsg/Fineweb-Edu-Chinese-V2.1", data_dir = "4_5", split=f"train[{offset_start}%:{offset_end}%]")
